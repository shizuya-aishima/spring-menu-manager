...
class MenuServiceTest {
  ...
  void getRootMenuItems_正常系() {
    ...
    assertTrue(
        rootItems.stream().allMatch(item -> item.getParentId() == null),
  ...
  void getSubMenuItems_正常系() {
    // 設定メニューのIDを取得
    Long settingsId =
        menuService.getRootMenuItems().stream()
    ...
    assertEquals("プロフィール設定", subItems.get(0).getName(), "サブメニュー項目の名前が「プロフィール設定」である必要があります");
  }
}
